...
<head>
    ...
    <link rel="icon" href="{{ asset('img/Regis.png') }}">
    ...
    <style>
        body {
            background: url('{{ asset('img/EL_Register.jpg') }}') no-repeat center center;
                        ...
                        <select name="rol_id" class="form-control form-control-user" required>
                            ...
                            @foreach($roles as $rol)
                                <option value="{{ $rol->id_rol }}" 
                                        {{ old('rol_id') == $rol->id_rol ? 'selected' : '' }}>
                                    {{ $rol->nombre_rol }}
                                </option>
                            @endforeach
                        </select>
            ...
            Swal.fire({
                ...
                html: `
                    <div style="text-align: center;">
                        ...
                        <h3 style="color: #28a745; margin-bottom: 10px;">¡Bienvenido, {{ session('usuario_nombre') }}!</h3>
                ...
                if (result.dismiss === Swal.DismissReason.timer) {
                    ...
                    window.location.href = "{{ route('login') }}"; /* "{{ route('login') }}?email={{ urlencode(session('usuario_email')) }}" */
                }
            });
        @endif
    </script>
